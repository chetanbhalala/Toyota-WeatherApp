...
class DashboardViewModel: ObservableObject {
    ...
    func getWeather(lat: Double, long: Double) async {
        ...
        MetricsManager.emitMetrics(metricName: "DashboardViewModel.getWeather.Time", time: ((endTime - startTime)))
    }
    ...
    func getSelectedLocation(currentLocation: CLLocationCoordinate2D?) async{
        ...
        let endTime = CFAbsoluteTimeGetCurrent() + kCFAbsoluteTimeIntervalSince1970
        MetricsManager.emitMetrics(metricName: "DashboardViewModel.getSelectedLocation.Time", time: ((endTime - startTime)))
    }
}
